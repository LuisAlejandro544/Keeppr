...
- **Identidad e Identificador de Paquete:** Su nombre base es **Keeppr** y su `applicationId` base es `com.keeppr` (`com.keeppr.beta` para el canal Release Beta y `com.keeppr.dev` para Canary Debug), garantizando almacenamiento limpio y profesional en `Android/data/com.keeppr*` sin dependencias ni referencias externas.
...
- **Lenguajes Compilados:**
  ...
  - **Rust (2021 Edition) & C Core:** Motor de cálculo, hashing SHA-256 criptográfico, cifrado simétrico grado militar AES-256 en modo CTR, derivación de claves PBKDF2-HMAC-SHA256 (10,000 iteraciones), código MAC HMAC-SHA256 y acelerador de rendimiento de notas (`app/src/main/rust`). Ejecuta en memoria nativa la extracción de métricas, conteo de palabras, resúmenes limpios de Markdown y filtrado/búsqueda insensible a mayúsculas. Se compila mediante la tarea Gradle `cargoBuild` y CMake para arquitecturas `aarch64-linux-android` (64 bits), `armv7-linux-androideabi` (32 bits / Android Go) y `x86_64-linux-android` (emuladores).
...
- **Integridad del Pipeline de Compilación y CI/CD:**
  ...
  - Flujo `build-release.yml` en GitHub Actions para compilación de APKs Release, disparado ante Pre-Releases con etiqueta terminada en `-b` (ej. `v0.1.0-b`). Con R8 Full Mode (`android.enableR8.fullMode=true`), poda de dependencias huérfanas (Retrofit, Moshi), eliminación de más de 75 idiomas en `resources.arsc` (`resourceConfigurations = ["es", "en"]`), exclusión de metadatos `META-INF/*.kotlin_module`, poda de Kotlin Intrinsics, eliminación de reglas estáticas redundantes de Compose y `extractNativeLibs = false` (evita duplicar librerías `.so` en disco ahorrando 3-5 MB de almacenamiento en 32 y 64 bits), con reglas estrictas de preservación JNI nativo en `proguard-rules.pro` preservando al 100% C++, Rust y Lua.
